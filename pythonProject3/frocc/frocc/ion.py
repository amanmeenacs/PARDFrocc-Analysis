...
def steel(majority=[0], frac=1, scaled=True, normalized=True):
    ...
    datadir = os.path.expanduser(f"~/data7")
    ...
    if os.path.isdir(datadir):
        ...
        # np.where((x_train == 0) | (x_train== None) , 1 )
        # if normalized:
...
x_train, y_train, x_val, y_val, x_test, y_test = steel(majority=[0], frac=1, scaled=True, normalized=True)