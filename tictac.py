...
def tictac(majority=["negative"], frac=1, scaled=True, normalized=True):
    import sklearn.datasets as skds

    datadir = os.path.expanduser(f"~/data2")
    ...
    if os.path.isdir(datadir):
        ...
        # print(22)
        x,y = skds.fetch_openml(data_id=50, return_X_y=True)
        # print(x)
        categories = np.array([0, 1])

        y.replace(['positive', 'negative'],[0, 1], inplace=True)
        x.replace(['o', 'x', 'b'], [0, 1, 2], inplace=True)
...
x_train, y_train, x_val, y_val, x_test, y_test = tictac(majority=[0], frac=1, scaled=True, normalized=True)